import { signIn, useSession } from "next-auth/react";
import { useState } from "react";
import Layout from "~/components/layout";
import { api } from "~/utils/api";

const Guestbook = () => {
  const [text, setText] = useState("");
  const { data: session } = useSession();

  const { mutateAsync, isLoading: mutationLoading } =
    api.sign.sign.useMutation();
  const { data, isLoading, refetch } = api.sign.getAll.useQuery();

  return (
    <Layout>
      <div className="relative flex gap-3">
        <input
          className="block w-full rounded-lg border border-gray-300 bg-gray-50 p-3 text-sm text-gray-900 transition-all focus:border-gray-400 focus:outline-gray-400 focus:ring-gray-400 dark:border-neutral-600 dark:bg-neutral-900 dark:text-white dark:placeholder-neutral-400 dark:outline-neutral-800 dark:focus:border-neutral-800 dark:focus:ring-gray-800"
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Add your signature"
        />
        {session ? (
          <button
            className="absolute right-1.5 top-1 my-auto flex items-center justify-center rounded-md bg-gray-100 px-3 py-1 text-lg transition-all hover:bg-gray-200 active:scale-95 dark:bg-neutral-700 dark:hover:bg-neutral-800"
            onClick={() => {
              mutateAsync({ text })
                .then(() => {
                  setText("");
                  console.log("Mutated Succesfully");
                  refetch();
                })
                .catch((e) => console.log(e));
            }}
          >
            {mutationLoading ? <Spinner /> : "Sign"}
          </button>
        ) : (
          <button
            className="absolute right-1.5 top-1 my-auto flex items-center justify-center rounded-md border-2 border-red-500 bg-gray-100 px-3 py-1 text-base transition-all hover:bg-gray-200 active:scale-95 dark:border-red-800 dark:bg-neutral-700 dark:hover:bg-neutral-800"
            onClick={() => void signIn()}
          >
            Sign In
          </button>
        )}
      </div>

      {isLoading ? (
        <div className="flex items-center justify-center p-10">
          <Spinner />
        </div>
      ) : (
        <div className="py-5">
          {data?.map((signature) => (
            <div key={signature.id} className="flex gap-1 px-1">
              <b className="text-gray-500 dark:text-neutral-500">
                {signature.author?.name}:
              </b>
              <p className="truncate">{signature.text}</p>
            </div>
          ))}
        </div>
      )}
    </Layout>
  );
};

export const Spinner = () => (
  <div role="status">
    <svg
      aria-hidden="true"
      className="mr-2 h-7 w-7 animate-spin fill-gray-300 text-gray-200 dark:text-gray-600"
      viewBox="0 0 100 101"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
        fill="currentColor"
      />
      <path
        d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
        fill="currentFill"
      />
    </svg>
    <span className="sr-only">Loading...</span>
  </div>
);

export default Guestbook;
